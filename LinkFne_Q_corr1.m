...
Ssigma=[1 0 0;...
        ...
        zeros(3,3);...
        0 0 1];                 %S1d
Sdelta =[eye(3) zeros(3,1)];   %S2d
Srho = [0;1;0;1;0;0;0;0;0];                  %   S1r
Sgamma = [zeros(1,3) 1]; %S2r
deltas = Sdelta*TVparam; 
D = diag(PositiveTrans(deltas)); 

gammas = Sgamma*TVparam;
[R,Pi,Psi] = MappingGammaToCorrelations(gammas);
...
Qdot = dvecQ_dvecD*dvecD_dSigma*dSigma_ddelta*ddelta_df +...
        dvecQ_dvecR*dvecR_drho*drho_dpi*dgamma_df;


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [R_t,Pi_t,Psi] = MappingGammaToCorrelations(gammas)

smoothConstantCorr = 1;%10^-1;%1;%10^-2; 
MaxValueCorr = 0.99; 
...
Pi_ = CorrTrans(gammas)';
Pi_t = eye(3) + [zeros(1,3); [Pi_ 0 0;0 0 0]] + [zeros(1,3); [Pi_ 0 0;0 0 0]]';

Psi =DerCorrTrans(Pi_t(2,1)); 
