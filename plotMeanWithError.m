function plotMeanWithShadedError95CI(data)
    ...
    row_std_error = std(data, 0, 2) ./ sqrt(size(data, 2));
    ...
    % Plot the shaded area (mean ± 1.96 * standard error)
    fill([time_points, fliplr(time_points)], ...
         [row_mean - ci95; flipud(row_mean + ci95)], ...
         [0.9 0.9 0.9], 'EdgeColor', 'none');
     
    % Re-plot the mean on top to make sure it's visible
    plot(time_points, row_mean, 'LineWidth', 2, 'Color', 'b');
