function run_tvp_mymodel_tot(j)
    j = str2double(j);
    cd("/rds/user/mx235/hpc-work/px_nav_ssm/mytvp/mymodel")
    ...
    names = ["psi_u","psi_p","psi_n","psi_T","theta_u","theta_p","eu","ep","en", "pu", "un","pn"]; % lower half of cov matrix
    OutParams = [MaxZero(1,EstimParams(1)),MaxZero(1,EstimParams(2)),MaxZero(1,EstimParams(3)),MaxZero(1,EstimParams(4)),MaxZero(-1,EstimParams(5)),MaxZero(-1,EstimParams(6)),EstimParams(7:12)];
    param_tab = table(names', OutParams', 'VariableNames', {'Names', 'Value'});
    save(strcat('param_est/',px_nav.isin{j}), 'param_tab');
