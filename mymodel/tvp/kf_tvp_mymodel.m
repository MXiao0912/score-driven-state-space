function [LL,Res] = kf_tvp_mymodel(Vpar,yy,initialF1)
...
kap_hes = MaxZero(1,Vpar(7));
c = Vpar(8:13);
A = diag(MaxZero(0.99,Vpar(14:19)));
B = diag(MaxZero(0.1,Vpar(20:25)));
...
[Q,Qdot] = LinkFne_Q_mymodel(initialF1);
...
initP = ((eye(m^2)-kron(T+(1e-6)*eye(m),T+(1e-6)*eye(m)))\eye(m^2))*Omega(:);
...
%         f_t(:,tt+1) = f_t(:,tt) + B_coeff*Score;
        ...
        % 2. Matrix Q
        [Q,Qdot] = LinkFne_Q_mymodel(f_t(:,tt+1));
        Res.Q(:,:,tt+1) =  Q;
